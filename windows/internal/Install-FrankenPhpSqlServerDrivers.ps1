...
$phpInfo = & $php -i
$phpVersionLine = $versionOutput | Where-Object { $_ -match '^PHP (\d+)\.(\d+)\.' } | Select-Object -First 1
if ($LASTEXITCODE -ne 0 -or -not $phpVersionLine -or $phpVersionLine -notmatch '^PHP (\d+)\.(\d+)\.') {
...
if (-not (Test-Path $phpIni -PathType Leaf)) {
    throw "The PHP configuration was not found at $phpIni"
}
$iniContent = [IO.File]::ReadAllText($phpIni)
foreach ($extension in @('pdo_sqlsrv', 'sqlsrv')) {
    $extensionPattern = "(?m)^\s*;?\s*extension\s*=\s*(?:php_)?$extension(?:\.dll)?\s*$"
    if ([Regex]::IsMatch($iniContent, $extensionPattern)) {
        $iniContent = [Regex]::Replace($iniContent, $extensionPattern, "extension=$extension")
    } else {
        $iniContent = $iniContent.TrimEnd() + [Environment]::NewLine + "extension=$extension" + [Environment]::NewLine
    }
}
