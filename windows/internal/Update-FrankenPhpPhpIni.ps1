...
if (-not $PSBoundParameters.ContainsKey('SourcePath')) {
    ...
    $SourcePath = Join-Path (Split-Path -Parent $PSScriptRoot) $sourceFileName
}
...
$iniContent = [IO.File]::ReadAllText($SourcePath)
...
[IO.File]::WriteAllText($destination, $iniContent, [Text.UTF8Encoding]::new($false))
